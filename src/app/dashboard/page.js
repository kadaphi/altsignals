  ...
  return (
    <div style={{ maxWidth:'1100px' }}>
      <style>{`
        ...
        .actions-grid { display:grid; grid-template-columns:repeat(8,1fr); gap:8px; }
      ...
      <div className="stats-grid">
        {[
          ...
          <div key={i} style={{ background:'#0F0F1A', border:'1px solid rgba(0,229,255,0.08)', padding:'20px', position:'relative', overflow:'hidden' }}>
            ...
            <div style={{ fontFamily:"'Space Grotesk',sans-serif", fontSize:'22px', fontWeight:'700', color:stat.color, marginBottom:'4px' }}>{stat.value}</div>
        ...
        <div className="actions-grid">
          {quickActions.map((action,i) => (
            <button key={i} onClick={() => router.push(action.path)}
              style={{ background:'#0F0F1A', border:'1px solid rgba(0,229,255,0.08)', padding:'16px 8px', display:'flex', flexDirection:'column', alignItems:'center', gap:'8px', cursor:'pointer', transition:'all 0.2s', fontFamily:'Inter,sans-serif' }}

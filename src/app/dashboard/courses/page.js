...
export default function CoursesPage() {
  ...
  return (
    <div style={{ maxWidth: '1100px' }}>
          ...
          return (
            ...
            >
              ...
              {isPurchased ? (
                ...
                <button onClick={() => handlePurchase(course.id)} disabled={purchasing === course.id} style={{ width: '100%', background: '#00E5FF', border: 'none', color: '#0A0A0F', padding: '12px', fontSize: '10px', fontWeight: '700', letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Inter,sans-serif', clipPath: 'polygon(6px 0%,100% 0%,calc(100% - 6px) 100%,0% 100%)', opacity: purchasing === course.id ? 0.6 : 1 }}>
      ...
      {/* 1-on-1 Coaching */}
      <div style={{ background: '#0F0F1A', border: '1px solid rgba(0,229,255,0.2)', padding: '40px', position: 'relative' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '24px' }}>
          <div style={{ flex: 1, minWidth: '280px' }}>
            ...
            <div style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: '28px', fontWeight: '700', color: '#E8E4DC', marginBottom: '12px' }}>1-on-1 Coaching Session</div>
            <p style={{ fontSize: '13px', color: '#8A8E99', lineHeight: '1.8', marginBottom: '0' }}>
              {coaching.coaching_description || 'Work directly with our head trader in a private one-on-one session. Get personalized feedback on your trading strategy, risk management and chart analysis. Sessions are tailored to your specific level and goals.'}
            </p>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '16px' }}>
            ...
            {coaching.coaching_link && (
              <a href={coaching.coaching_link} target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#7B61FF', color: '#ffffff', padding: '14px 28px', fontSize: '11px', fontWeight: '700', letterSpacing: '2px', textTransform: 'uppercase', textDecoration: 'none', clipPath: 'polygon(6px 0%,100% 0%,calc(100% - 6px) 100%,0% 100%)' }}>
                Book Session →
              </a>
            )}
          </div>

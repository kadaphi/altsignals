  ...
  return (
    <div style={{ maxWidth:'1200px' }}>
      <style>{`
        ...
        .users-table-row { display:grid; grid-template-columns:2fr 2fr 1fr 1fr 1fr 80px; gap:12px; padding:14px 16px; border-bottom:1px solid rgba(0,229,255,0.04); align-items:center; }
        .users-table-header { display:grid; grid-template-columns:2fr 2fr 1fr 1fr 1fr 80px; gap:12px; padding:12px 16px; border-bottom:1px solid rgba(0,229,255,0.08); }
        @media(max-width:768px){
          .users-table-header { display:none !important; }
          .users-table-row { grid-template-columns:1fr !important; padding:14px 16px !important; gap:6px !important; border-bottom:1px solid rgba(0,229,255,0.08) !important; }
        }
      `}</style>
      ...
      <input
        style={{ width:'100%', background:'#0F0F1A', border:'1px solid rgba(0,229,255,0.15)', padding:'12px 16px', color:'#E8E4DC', fontFamily:'Inter,sans-serif', fontSize:'13px', outline:'none', marginBottom:'24px' }}
      ...
      <div style={{ background:'#0F0F1A', border:'1px solid rgba(0,229,255,0.08)' }}>
        <div className="users-table-header" style={{ fontSize:'9px', fontWeight:'700', letterSpacing:'2px', textTransform:'uppercase', color:'#8A8E99' }}>
          <div>Name</div>
          <div>Email</div>
          <div>Deposit</div>
          <div>Withdraw</div>
        ...
        {filtered.map((u, i) => (
          <div key={i} className="users-table-row">
            ...
            <div>
              <div style={{ fontSize:'9px', color:'#8A8E99', marginBottom:'2px' }}>Deposit</div>
              <div style={{ fontSize:'12px', color:'#00E5FF', fontWeight:'600' }}>${Number(u.deposit_balance || 0).toFixed(2)}</div>
            </div>
            <div>
              <div style={{ fontSize:'9px', color:'#8A8E99', marginBottom:'2px' }}>Withdraw</div>
              <div style={{ fontSize:'12px', color:'#00FF88', fontWeight:'600' }}>${Number(u.withdrawal_balance || 0).toFixed(2)}</div>
            </div>
            ...
            <button onClick={() => { setSelectedUser(u); setEditForm({ deposit_balance: u.deposit_balance, withdrawal_balance: u.withdrawal_balance, account_level: u.account_level, is_banned: u.is_banned }) }}
              style={{ background:'rgba(0,229,255,0.08)', border:'1px solid rgba(0,229,255,0.2)', color:'#00E5FF', padding:'6px 12px', fontSize:'10px', fontWeight:'600', cursor:'pointer', fontFamily:'Inter,sans-serif' }}>
              Edit
            </button>
          </div>
        ))}
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.85)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000, padding:'20px' }}>
          <div style={{ background:'#0F0F1A', border:'1px solid rgba(0,229,255,0.2)', padding:'32px', maxWidth:'480px', width:'100%', position:'relative', maxHeight:'90vh', overflowY:'auto' }}>
            ...
            <div style={{ marginBottom:'24px', display:'flex', alignItems:'center', gap:'12px' }}>
              <input type="checkbox" id="banned" checked={editForm.is_banned || false}

        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: '16px' }}>
          {challenges.map((c, i) => (
            <div key={i} style={{ background: '#0F0F1A', border: '1px solid rgba(0,229,255,0.08)', padding: '24px', position: 'relative' }}>
              ...
              <div style={{ fontSize: '11px', color: '#8A8E99', marginBottom: '12px', lineHeight: '1.6' }}>{c.description}</div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px' }}>
                <span style={{ color: '#8A8E99' }}>Entry: <span style={{ color: '#E8E4DC', fontWeight: '600' }}>${c.entry_fee}</span></span>
                <span style={{ color: c.is_active ? '#00FF88' : '#FF4444', fontWeight: '600' }}>{c.is_active ? 'ACTIVE' : 'INACTIVE'}</span>
              </div>
            </div>
          ))}
        </div>

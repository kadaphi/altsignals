      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '16px', height: '600px' }}>
        <div style={{ background: '#0F0F1A', border: '1px solid rgba(0,229,255,0.08)', overflowY: 'auto' }}>
          ...
          {sessions.length === 0 ? (
            ...
            <div key={i} onClick={() => setSelectedSession(s)} style={{ padding: '16px', borderBottom: '1px solid rgba(0,229,255,0.04)', cursor: 'pointer', background: selectedSession?.id === s.id ? 'rgba(0,229,255,0.06)' : 'none', borderLeft: selectedSession?.id === s.id ? '2px solid #00E5FF' : '2px solid transparent' }}>
          ...
          {!selectedSession ? (
            ...
            <>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {messages.map((msg, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: msg.is_admin ? 'flex-end' : 'flex-start' }}>
                    <div style={{ maxWidth: '70%', background: msg.is_admin ? 'rgba(0,229,255,0.15)' : '#111320', border: `1px solid ${msg.is_admin ? 'rgba(0,229,255,0.3)' : 'rgba(0,229,255,0.08)'}`, padding: '10px 14px' }}>
                      ...
                      <div style={{ fontSize: '9px', color: '#8A8E99', marginTop: '4px' }}>{new Date(msg.created_at).toLocaleTimeString()}</div>
                    </div>
                  </div>
                ))}
              ...
              <div style={{ padding: '16px', borderTop: '1px solid rgba(0,229,255,0.08)', display: 'flex', gap: '12px' }}>
                ...
                />
                <button onClick={handleReply} disabled={sending || !reply.trim()} style={{ background: '#00E5FF', border: 'none', color: '#0A0A0F', padding: '12px 24px', fontSize: '10px', fontWeight: '700', letterSpacing: '2px', textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'Inter,sans-serif', opacity: sending || !reply.trim() ? 0.6 : 1 }}>
                  {sending ? '...' : 'Send'}
                </button>
              </div>
            </>
